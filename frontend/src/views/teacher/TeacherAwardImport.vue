<template>
  <div class="page-container">
    <div class="page-header">
      <div>
        <h2 class="page-title-text">奖项导入</h2>
        <p class="page-desc">为学生录入获奖记录，支持单条录入和批量导入</p>
      </div>
      <div class="flex gap-8">
        <button class="btn btn-secondary" @click="openBatchModal">批量导入</button>
        <button class="btn btn-primary" @click="openAddModal">录入奖项</button>
      </div>
    </div>

    <!-- 筛选 -->
    <div class="filter-bar card">
      <input class="form-input" v-model="filter.awardName" placeholder="奖项名称" @keyup.enter="loadData" style="width: 160px" />
        ...
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr v-if="list.length === 0">
              <td colspan="8" class="empty-cell">暂无奖项数据</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div class="modal-overlay" v-if="showFormModal" @click.self="showFormModal = false">
      <div class="modal-content" style="max-width: 520px">
        <div class="modal-header">
          <h3 class="modal-title">{{ editing ? '编辑奖项' : '录入奖项' }}</h3>
        ...
        <div class="modal-body">
          <div class="form-grid">
            ...
            <div class="form-group">
              ...
              <input class="form-input" v-model="form.awardName" placeholder="奖项名称" required />
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
              ...
              <input class="form-input" v-model="form.issuingAuthority" placeholder="颁发机构" />
            </div>
            <div class="form-group" style="grid-column: 1 / -1">
              <label class="form-label">描述</label>
              <input class="form-input" v-model="form.description" placeholder="奖项描述" />
            </div>
          </div>
        </div>
    ...
    <div class="modal-overlay" v-if="showBatchModal" @click.self="showBatchModal = false">
      <div class="modal-content" style="max-width: 700px">
        <div class="modal-header">
          <h3 class="modal-title">批量导入奖项</h3>
          <button class="btn-icon" @click="showBatchModal = false">✕</button>
        </div>
        <div class="modal-body">
          <p class="text-caption" style="color: var(--text-tertiary)">
            请在下方文本框中粘贴奖项数据，每行一条，字段用 Tab 或逗号分隔。<br/>
            格式：学号, 奖项名称, 奖项级别, 获奖日期(YYYY-MM-DD), 颁发机构, 描述
          </p>
          <textarea class="form-input" v-model="batchText" rows="10" style="font-family: var(--font-mono); font-size: 12px" placeholder="20210001	全国数学竞赛一等奖	国家级	2024-06-15	教育部	&#10;20210002	优秀学生干部	校级	2024-09-01	学校"></textarea>
...
async function handleSave() {
  if (!form.value.awardName) { alert('请填写奖项名称'); return }
  try {
...
async function handleBatchImport() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (cols.length < 2) {
      batchErrors.value.push(`第 ${i + 1} 行：字段不足（至少需要学号、奖项名称）`)
      continue
    }
    ...
    awards.push({
      studentId: sid, awardName: cols[1], awardLevel: cols[2] || null,
